...
const ProjectCard: React.FC<ProjectCardProps> = ({ title, thumbnail, githubLink, description }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Front Side */}
        <div className={`absolute inset-0 bg-cover bg-center ${flipped ? 'hidden' : 'block'}`}
             style={{ backgroundImage: `url(${thumbnail})` }}>
          <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center text-white">
            <h2 className="text-2xl font-semibold mb-2">{title}</h2>
            <a href={githubLink} target="_blank" rel="noopener noreferrer" className="text-teal-400 text-3xl hover:text-teal-300">
              <FaGithub />
            </a>
          </div>
        </div>
