      ...
      <aside className={`lg:w-64 bg-gray-800 text-white fixed h-full flex flex-col transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out z-40`}>
        ...
        <nav className="flex-1">
          <ul className="flex flex-col space-y-1">
            <li><Link to="/" className="flex items-center p-4 hover:bg-teal-700" onClick={toggleSidebar}><FaHome className="mr-3" /> Home</Link></li>
            <li><Link to="/about" className="flex items-center p-4 hover:bg-teal-700" onClick={toggleSidebar}><FaUser className="mr-3" /> About</Link></li>
            ...
            <li><a href="pdf/jobresume.pdf" target="_blank" rel="noopener noreferrer" className="flex items-center p-4 hover:bg-teal-700" onClick={toggleSidebar}><FaFileAlt className="mr-3" /> Resume</a></li>
          </ul>
        </nav>
